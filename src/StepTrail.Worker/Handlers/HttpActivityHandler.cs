...
/// Secret placeholders ({{secrets.name}}) in URL, header values, and body are resolved via SecretResolver
/// before the request is made — secrets are never stored in the step config itself.
/// On non-2xx response the handler throws, which causes the worker to fail the step and apply the retry policy.
/// On success the response status code and body are stored as the step's JSON output.
/// </summary>
public sealed class HttpActivityHandler : IStepHandler
{
    ...
    public async Task<StepResult> ExecuteAsync(StepContext context, CancellationToken ct)
    {
        ...
        // Resolve {{secrets.name}} placeholders before building the request.
        var url = await _secrets.ResolveAsync(config.Url, ct) ?? config.Url;
        ...
        if (config.Headers is not null)
            ...
            {
                var resolvedValue = await _secrets.ResolveAsync(value, ct) ?? value;
        ...
        var body = await _secrets.ResolveAsync(rawBody, ct);
        if (body is not null)
        ...
        // Capture status + body as structured output regardless of success/failure.
